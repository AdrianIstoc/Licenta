...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.select_button = tk.Button(self.toolbar, text="Selectează", command=self.toggle_area_selection)
        ...
        self.option_label = tk.Label(self.side_menu, text="Descarcă:")
        ...
        self.prediction_label=tk.Label(self.side_menu, text="Luni prezise:")
    ...
    def prediction(self):
        if self.downloaded_data is None or self.downloaded_data["results"]==[]:
            self.show_status(text="Nu există date pentru predicție!", duration=3000)
            return
        elif self.downloaded_data["option"]=="RGB":
            self.show_status(text="Nu se pot prezice date RGB!", duration=3000)
            return
    ...
    def show_image(self):
        if self.downloaded_data is None or self.downloaded_data["results"]==[]:
            self.show_status(text="Nu există date!", duration=3000)
            return
    ...
    def graph_data(self):
        if self.downloaded_data is None or self.downloaded_data["results"]==[]:
            ...
            self.show_status(text="Nu se poate face un grafic pentru RGB!", duration=3000)
            return
    ...
    def download_selected_data(self):
        if self.download_option.get() == "None":
            self.show_status(text="Nu s-a ales o opțiune pentru descărcarea datelor!", duration=3000)
            return
        elif self.starting_point is None:
            self.show_status(text="Nu s-a selectat o zonă!", duration=3000)
            return
        elif self.start_date is None:
            self.show_status(text="Nu s-a selectat o perioadă de timp!", duration=3000)
            return
        elif self.start_date > self.end_date:
            self.show_status(text="Data de început trebuie să fie înainte de data de final!", duration=3000)
            return
    ...
    def confirmDate(self):
        ...
        self.show_status(text="Perioadă de timp selectată!", duration=3000)
    ...
    def toggle_area_selection(self):
        ...
        if self.selecting_area:
            ...
            self.show_status("Selectează: folosește clic dreapta pentru a selecta!", duration=3000)
        else:
            self.select_button.config(text="Selectează", relief=tk.RAISED)
            self.disable_selection()
    ...
    def on_mouse_up(self, event):
        ...
        if self.starting_point is not None:
            self.show_status("Selecție salvată.", duration=3000)
